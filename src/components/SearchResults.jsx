...
import { YOUTUBE_SEARCH_API, YOUTUBE_VIDEO_DETAILS_API } from '../utils/constants';
...
const SearchResults = () => {
  ...
  const fetchSearchResults = async () => {
    ...
    //   setVideos(data.items || []);
            ...
            const searchResponse = await fetch(`${YOUTUBE_SEARCH_API}&q=${encodeURIComponent(searchQuery)}`);
            ...
            const detailsResponse = await fetch(`${YOUTUBE_VIDEO_DETAILS_API}&id=${videoIds}`);
            if (!detailsResponse.ok) throw new Error(`Video Details API error! Status: ${detailsResponse.status}`);
